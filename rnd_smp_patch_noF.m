function [Xh, Xl] = rnd_smp_patch_noF(img_path, type, patch_size, num_patch, upscale)
...
for ii = 1:img_num,
    ...
    im = imread(fullfile(img_path, img_dir(ii).name));
    [H, L] = sample_patches_noF(im, patch_size, patch_num, upscale);
    Xh = [Xh, H];
    Xl = [Xl, L];
end

patch_path = ['Training/rnd_patches_noF_' num2str(patch_size) '_' num2str(num_patch) '_s' num2str(upscale) '.mat'];
